...
export default function ApiDocsPage() {
  ...
  const endpoints: { category: string; description?: string; items: Endpoint[]; notes?: { title: string; body: string[] } }[] = [
    ...
    },
    {
      ...
      description: 'Connect, manage, and communicate via WhatsApp. Messages and conversations use the standard endpoints filtered by provider=whatsapp. Auto-syncs contacts and recent messages on connect.',
      items: [
        {
          method: 'GET',
          path: '/integrations/whatsapp/status',
          description: 'Get WhatsApp connection status.',
          auth: 'X-API-Key',
          response: { status: 'ready', phoneNumber: '+1234567890', message: 'Connected' },
        },
        {
          method: 'POST',
          path: '/integrations/whatsapp/connect',
          description: 'Initialize WhatsApp connection (generates QR code).',
          auth: 'X-API-Key',
          response: { status: 'qr_ready', message: 'Scan QR code...' },
        },
        {
          method: 'GET',
          path: '/integrations/whatsapp/qr',
          description: 'Get QR code for WhatsApp Web pairing.',
          auth: 'X-API-Key',
          response: { qrCode: 'data:image/png;base64,...', status: 'qr_ready' },
        },
        {
          method: 'DELETE',
          path: '/integrations/whatsapp/disconnect',
          description: 'Disconnect WhatsApp and cleanup session.',
          auth: 'X-API-Key',
          response: { success: true, message: 'Disconnected' },
        },
        {
          method: 'GET',
          path: '/integrations/whatsapp/health',
          description: 'Check WhatsApp service availability.',
          auth: 'X-API-Key',
          response: { available: true },
        },
        {
          method: 'POST',
          path: '/integrations/whatsapp/send',
          description: 'Send a WhatsApp message.',
          auth: 'X-API-Key',
          body: { to: '+1234567890', body: 'Hello!' },
          response: { success: true, messageId: 'wa_msg_123' },
        },
        {
          method: 'GET',
          path: '/conversations?provider=whatsapp',
          description: 'List WhatsApp conversations with names, avatars, and unread counts.',
          auth: 'X-API-Key',
          response: { data: [{ id: 'uuid', contactName: 'John', avatarUrl: 'https://...', unreadCount: 3, lastMessage: 'Hello', lastMessageAt: '...', isGroup: false, provider: 'whatsapp' }] },
        },
        {
          method: 'GET',
