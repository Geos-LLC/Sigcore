...
export class AccountStoreService {
  ...
  private load() {
    try {
      ...
      for (const acc of parsed) {
        // Backfill new fields on older records to keep restart-safety.
        if (acc.reconnectAttempts == null) acc.reconnectAttempts = 0;
  ...
  setStatus(accountId: string, status: TelegramAccountStatus, lastError?: string) {
    ...
    if (status === 'connected') {
      acc.lastConnectedAt = acc.updatedAt;
  ...
  recordPing(accountId: string) {
    const acc = this.accounts.get(accountId);
    if (!acc) return;
    acc.lastPingAt = new Date().toISOString();
    this.persist();
  }
  ...
  decryptGramjsSession(acc: StoredTelegramAccount): string {
    if (!acc.gramjsSessionEncrypted) throw new Error('no_gramjs_session');
    return this.encryption.decrypt(acc.gramjsSessionEncrypted);
  }
}
