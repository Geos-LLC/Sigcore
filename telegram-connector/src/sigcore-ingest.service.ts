...
/**
 * Forwards normalized inbound messages to Sigcore core. Mirrors the WhatsApp
 * connector shape: POST /webhooks/telegram/inbound with `x-webhook-key` for
 * service-to-service auth.
 *
 * Called by the drainer — never by the webhook handler directly, so the
 * durable event store is always the source of truth for in-flight events.
 */
@Injectable()
