...
/**
 * Thin Telegram Bot API client. Only the calls the connector needs. Avoids
 * pulling in node-telegram-bot-api or grammY — Sigcore drives the webhook /
 * polling lifecycle and we want the dependency surface minimal.
 */
...
function scrubMessage(msg: string): string {
  ...
  // into an axios error string. The full /bot<token>/ path appears in some
  // error messages, hence the broader pattern here.
