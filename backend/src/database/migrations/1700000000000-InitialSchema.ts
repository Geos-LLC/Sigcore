...
export class InitialSchema1700000000000 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(`DO $$ BEGIN
      CREATE TYPE "provider_type_enum" AS ENUM ('openphone','twilio','telegram');
    ...
    await queryRunner.query(`DO $$ BEGIN
      CREATE TYPE "channel_type_enum" AS ENUM ('sms','whatsapp','telegram','voice');
    EXCEPTION WHEN duplicate_object THEN null; END $$`);

    await queryRunner.query(`DO $$ BEGIN
      CREATE TYPE "sender_status_enum" AS ENUM ('active','inactive','pending','error');
    EXCEPTION WHEN duplicate_object THEN null; END $$`);

    await queryRunner.query(`DO $$ BEGIN
      CREATE TYPE "sender_mode_enum" AS ENUM ('shared','dedicated','openphone');
    EXCEPTION WHEN duplicate_object THEN null; END $$`);
    ...
    await queryRunner.query(`DO $$ BEGIN
      CREATE TYPE "message_status_enum" AS ENUM ('pending','sent','delivered','failed');
    EXCEPTION WHEN duplicate_object THEN null; END $$`);

    await queryRunner.query(`DO $$ BEGIN
      CREATE TYPE "call_direction_enum" AS ENUM ('in','out');
    EXCEPTION WHEN duplicate_object THEN null; END $$`);

    await queryRunner.query(`DO $$ BEGIN
      CREATE TYPE "call_status_enum" AS ENUM ('completed','missed','voicemail','cancelled');
    EXCEPTION WHEN duplicate_object THEN null; END $$`);

    await queryRunner.query(`DO $$ BEGIN
      CREATE TYPE "identity_status_enum" AS ENUM ('active','inactive','unverified');
    EXCEPTION WHEN duplicate_object THEN null; END $$`);
    ...
    await queryRunner.query(`DO $$ BEGIN
      CREATE TYPE "phone_number_provider_enum" AS ENUM ('twilio','openphone','whatsapp');
    EXCEPTION WHEN duplicate_object THEN null; END $$`);

    await queryRunner.query(`DO $$ BEGIN
      CREATE TYPE "phone_number_allocation_status_enum" AS ENUM ('active','inactive','pending');
    EXCEPTION WHEN duplicate_object THEN null; END $$`);

    await queryRunner.query(`DO $$ BEGIN
      CREATE TYPE "phone_number_order_type_enum" AS ENUM ('purchase','release');
    EXCEPTION WHEN duplicate_object THEN null; END $$`);

    await queryRunner.query(`DO $$ BEGIN
      CREATE TYPE "phone_number_order_status_enum" AS ENUM ('pending','provisioning','active','releasing','released','failed','cancelled');
    EXCEPTION WHEN duplicate_object THEN null; END $$`);

    await queryRunner.query(`DO $$ BEGIN
      CREATE TYPE "pricing_type_enum" AS ENUM ('fixed_markup','percentage_markup','fixed_price');
    EXCEPTION WHEN duplicate_object THEN null; END $$`);
    ...
    await queryRunner.query(`DO $$ BEGIN
      CREATE TYPE "sms_status_enum" AS ENUM ('queued','sent','delivered','undelivered','failed','received');
    EXCEPTION WHEN duplicate_object THEN null; END $$`);

    await queryRunner.query(`DO $$ BEGIN
      CREATE TYPE "phone_number_type_enum" AS ENUM ('BOT','DEDICATED');
    EXCEPTION WHEN duplicate_object THEN null; END $$`);

    await queryRunner.query(`DO $$ BEGIN
      CREATE TYPE "webhook_subscription_status_enum" AS ENUM ('active','inactive','paused');
    EXCEPTION WHEN duplicate_object THEN null; END $$`);

    await queryRunner.query(`DO $$ BEGIN
      CREATE TYPE "call_connect_mode_enum" AS ENUM ('AGENT_FIRST','PARALLEL');
    EXCEPTION WHEN duplicate_object THEN null; END $$`);

    await queryRunner.query(`DO $$ BEGIN
      CREATE TYPE "agent_strategy_enum" AS ENUM ('OWNER','ROUND_ROBIN','ON_DUTY');
    EXCEPTION WHEN duplicate_object THEN null; END $$`);

    await queryRunner.query(`DO $$ BEGIN
      CREATE TYPE "caller_id_strategy_enum" AS ENUM ('BOT_NUMBER','BUSINESS_NUMBER');
    EXCEPTION WHEN duplicate_object THEN null; END $$`);

    await queryRunner.query(`DO $$ BEGIN
      CREATE TYPE "agent_voicemail_mode_enum" AS ENUM ('TTS','SPEAK');
    EXCEPTION WHEN duplicate_object THEN null; END $$`);

    await queryRunner.query(`DO $$ BEGIN
      CREATE TYPE "session_status_enum" AS ENUM ('CREATED','CALLING_AGENT','AGENT_ANSWERED','AGENT_ACCEPTED','CALLING_LEAD','LEAD_ANSWERED','BRIDGED','ENDED','FAILED','CANCELED');
    EXCEPTION WHEN duplicate_object THEN null; END $$`);

    await queryRunner.query(`DO $$ BEGIN
      CREATE TYPE "call_connect_provider_enum" AS ENUM ('TWILIO');
    EXCEPTION WHEN duplicate_object THEN null; END $$`);
    ...
    await queryRunner.query(`
      CREATE TABLE IF NOT EXISTS "communication_conversations" (
        ...
        "created_at"               timestamptz NOT NULL DEFAULT now(),
        "updated_at"               timestamptz NOT NULL DEFAULT now(),
        CONSTRAINT "FK_conversations_sender" FOREIGN KEY ("sender_id") REFERENCES "senders"("id") ON DELETE SET NULL
      )`);
    ...
    await queryRunner.query(`
      CREATE TABLE IF NOT EXISTS "communication_messages" (
        ...
        "metadata"            jsonb,
        "created_at"          timestamptz NOT NULL DEFAULT now(),
        CONSTRAINT "FK_messages_conversation" FOREIGN KEY ("conversation_id") REFERENCES "communication_conversations"("id") ON DELETE CASCADE,
        CONSTRAINT "FK_messages_from_identity" FOREIGN KEY ("from_identity_id") REFERENCES "contact_identities"("id") ON DELETE SET NULL,
        CONSTRAINT "FK_messages_to_identity" FOREIGN KEY ("to_identity_id") REFERENCES "contact_identities"("id") ON DELETE SET NULL
      )`);
    ...
    await queryRunner.query(`
      CREATE TABLE IF NOT EXISTS "communication_calls" (
        ...
        CONSTRAINT "FK_calls_conversation" FOREIGN KEY ("conversation_id") REFERENCES "communication_conversations"("id") ON DELETE CASCADE
      )`);
    ...
    await queryRunner.query(`
      CREATE TABLE IF NOT EXISTS "tenant_phone_numbers" (
        ...
        "created_at"             timestamptz NOT NULL DEFAULT now(),
        "updated_at"             timestamptz NOT NULL DEFAULT now(),
        CONSTRAINT "FK_tenant_phone_numbers_tenant" FOREIGN KEY ("tenant_id") REFERENCES "tenants"("id") ON DELETE CASCADE
      )`);
    ...
    await queryRunner.query(`
      CREATE TABLE IF NOT EXISTS "phone_number_pricing" (
        "id"                         uuid DEFAULT gen_random_uuid() PRIMARY KEY,
        "workspace_id"               varchar NOT NULL,
    ...
    await queryRunner.query(`
      CREATE TABLE IF NOT EXISTS "phone_number_orders" (
        "id"                    uuid DEFAULT gen_random_uuid() PRIMARY KEY,
        "workspace_id"          varchar NOT NULL,
        "tenant_id"             varchar,
        "phone_number"          varchar,
        ...
        CONSTRAINT "FK_phone_number_orders_tenant" FOREIGN KEY ("tenant_id") REFERENCES "tenants"("id") ON DELETE SET NULL,
        CONSTRAINT "FK_phone_number_orders_tpn" FOREIGN KEY ("tenant_phone_number_id") REFERENCES "tenant_phone_numbers"("id") ON DELETE SET NULL
      )`);
    ...
    await queryRunner.query(`
      CREATE TABLE IF NOT EXISTS "call_connect_settings" (
    ...
    await queryRunner.query(`
      CREATE TABLE IF NOT EXISTS "call_connect_sessions" (
        "id"                      uuid DEFAULT gen_random_uuid() PRIMARY KEY,
        "business_id"             varchar NOT NULL,
        "tenant_id"               varchar,
