...
export class CallConnectService {
  ...
  async startSession(
    ...
    // selection later will surface any real config gap).
    ...
    if (this.providerContextResolver) {
      try {
        ...
        if (!(err instanceof NotFoundException)) {
          // 403 cross-tenant / 409 ambiguous / 409 provider mismatch — those
          // are security-critical. Fail closed rather than starting a Call
          // Connect session that can't be reliably routed later.
          throw err;
        }
        this.logger.warn(
  ...
  private async getTwilioClient(
    ...
    // Kept as-is: tenant_integrations is the per-tenant provisioning table
    // and this table is intentionally NOT what the resolver consults.
    if (tenantId) {
    ...
    // resolver isn't wired (test builders).
    ...
    if (this.providerContextResolver) {
      try {
        ...
        // NotFound -> fall through to the pre-Phase-3a lookup to preserve
        // the existing error surface (a thrown Error, not a NestJS 404).
        // Other resolver errors (403/409) are security-critical and MUST
        // propagate.
        if (!(err instanceof NotFoundException)) {
          throw err;
        }
